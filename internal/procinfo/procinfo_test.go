...
// permanently unidentified — Observe has to fall back to a direct OS
...
func TestObserve_FallsBackToOSQueryWhenNameAndParentBothMissing(t *testing.T) {
	...
	if got.Name == "" {
		t.Fatalf("expected Observe's OS-query fallback to resolve our own PID %d, got empty Name", selfPID)
	}
	if got.ImagePath == "" {
